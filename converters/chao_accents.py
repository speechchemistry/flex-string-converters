...
#
#   Reverses converters/chao_tones.py: given base text followed by the Chao
#   tone letters extracted from it, places tone-accent diacritics back onto
...
# The inverse of converters/chao_tones.py's ACCENT_TO_TONE_LETTERS. Kept as
# its own copy rather than an import, since a converter must stand alone as
# a FLEx Process; guarded against drift from that file's table by a test.
TONE_LETTERS_TO_ACCENT = {'˥':'̋', # ő
                   '˦':'́', # ó
                   '˧':'̄', # ō
                   '˨':'̀', # ò
                   '˩':'̏', # ȍ
...
# Tone-bearing vowels and syllabic marks: copies of chao_tones.py's tables,
# guarded against drift by a test that checks them against that file's.
...
def _word_units(word):
    ...
    # a vowel run without starting a unit. Mirrors chao_tones.py's
    # _tone_bearing_units(), but keeps cluster positions so an accent can be
...
def _place_word(word, tone_group_string):
    ...
    for unit_indices, group in zip(units, groups):
        ...
        if unit_size == 1 and group in TONE_LETTERS_TO_ACCENT:
            # A single tone-bearing cluster: the whole group is one accent.
            clusters[unit_indices[0]].append(TONE_LETTERS_TO_ACCENT[group])
        elif len(group) == 1:
            # A level tone spread over several vowels repeats on each.
            accent = TONE_LETTERS_TO_ACCENT[group]
            for index in unit_indices:
                clusters[index].append(accent)
        elif len(group) == unit_size:
            # One tone letter per vowel: one accent per cluster, in order.
            accents = []
            for letter in group:
                if letter not in TONE_LETTERS_TO_ACCENT:
                    return None
                accents.append(TONE_LETTERS_TO_ACCENT[letter])
            for index, accent in zip(unit_indices, accents):
                clusters[index].append(accent)
        else:
            return None
...
def apply_chao_letters(base_text, tone_letters):
    # The placement engine, counterpart of chao_tones.py's
    # extract_chao_letters(): places tone_letters back onto base_text's
    # tone-bearing units, word by word. Returns None if the words and tone
    # groups don't correspond 1:1, or a word's groups don't fit its units --
    # useful on its own when spelling and tone letters already come from two
    # separate fields (e.g. a FlexTools module reading spelling and Pitch).
...
def convert(input_string): # function is named "convert" so it can be used as an SIL Flex Process
    ...
    if not match:
        return input_string
    placed = apply_chao_letters(match.group('base'), match.group('tones'))
    if placed is None:
        return input_string
    return placed
...
def parse_arguments():
    """Places Chao tone letters back onto their base text as accent notation"""
    parser = argparse.ArgumentParser(
        description="Place Chao tone letters back onto their base text as "
                    "accent notation, e.g. nəjɛt ˨ ˨˧ -> nə̀jɛ᷅t.")
...
def use_utf8(*streams):
    # The output is IPA accent notation, so don't leave the encoding to the
