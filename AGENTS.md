...
- Give each converter a command line interface under `if __name__ == '__main__':`, following [CLI Script Conventions](#cli-script-conventions), so the conversion is usable outside FlexTools without any FLEx installation — **except a forwarding wrapper as above**, when its sibling's own CLI already exposes the identical behaviour (e.g. behind a flag): a second, duplicate CLI would add nothing worth maintaining.
