...
- Test each converter's `convert()` directly — it needs no FLEx project, so those tests run on any platform. `tests/conftest.py` puts `converters/` on `sys.path` so `from diacritics2chao import convert` works.
