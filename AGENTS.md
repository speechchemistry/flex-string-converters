...
- Keep the module thin. Domain logic belongs in `converters/`; the module's job is reading the field, calling `convert()`, and writing the result. `__Template_converter_module.py` is the starting point — copy it and change the marked places.
