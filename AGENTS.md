...
- A converter otherwise stands alone and never imports another converter's module — **except a pure forwarding wrapper**, whose only reason to exist is giving a raw FLEx Process (which always calls a bare `Convert(input_string)` with no keyword arguments, and can only choose *which file* to point at) a second file that either presets one of its sibling's parameters or exposes one of its sibling's other public functions as its own `Convert()`. Import the sibling by adding its own directory to `sys.path` from `__file__` before importing it, the same technique [FlexTools Module Conventions](#flextools-module-conventions) uses for a module reaching its project's `converters/`. See `chao-tone-letters/converters/diacritics2chao_attached.py` and `diacritics2chao_tone_letters_only.py` for the worked examples.
...
- Give each converter a command line interface under `if __name__ == '__main__':`, following [CLI Script Conventions](#cli-script-conventions), so the conversion is usable outside FlexTools without any FLEx installation — **except a forwarding wrapper as above**, when the behaviour it exposes is already reachable outside FlexTools another way: its sibling's own CLI (e.g. behind a flag), or a plain Python import of the sibling function it forwards to. A second, duplicate CLI would add nothing worth maintaining.
