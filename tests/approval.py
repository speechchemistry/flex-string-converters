...
#   chao-tone-letters/tests/test_diacritics2chao_cli.py for the worked example
#   that first introduced it.
#
#   Each project keeps its own tests/fixtures/ directory, so every function
#   here takes that project's tests/ directory explicitly rather than
#   assuming a single repo-wide location.
#
...
#   <project>/tests/fixtures/<converter>/received/ either way, and the
#   failure message gives the exact command to promote it once it has been
#   read and found correct. Nothing is ever written into approved/
#   automatically.
#
...
def fixtures_dir(tests_dir, converter_name):
    return tests_dir / "fixtures" / converter_name


def input_fixtures(tests_dir, converter_name):
    ...
    fixtures = fixtures_dir(tests_dir, converter_name)
...
def _promote_command(tests_dir, converter_name, input_path, received_path):
    approved_path = fixtures_dir(tests_dir, converter_name) / "approved" / f"{input_path.stem}.approved.txt"
    return (
...
def assert_approved(tests_dir, converter_name, input_path, approved_path, actual):
    received_dir = fixtures_dir(tests_dir, converter_name) / "received"
    ...
    if not approved_path.exists():
        ...
        pytest.fail(
            ...
            f"  {_promote_command(tests_dir, converter_name, input_path, received_path)}"
        )
