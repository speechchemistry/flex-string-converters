...
3. The stripped string is matched against a token lexicon using a finite-state transducer (built with [pynini](https://pypi.org/project/pynini/)), tokenised by maximal munch:
   ...
   - `ɲ` maps to `nj` — the phonology sketch states this as a reanalysis. The real FLEx export originally had three words transcribed with phonemic `ɲ` against eleven with phonemic `nj`, never conflated; reviewed against the source data (2026-09-01), two (`ɲápsə́` 'hesitation', `ɲúnə̀` 'bitterness') confirmed the rule and are now expected in the `real_flex_export` fixture as `njápsə́`/`njúnə̀`. The third (`ɲo᷆k` 'to weave') turned out to be unrelated to this rule — its `ɲ` was a transcription error for plain `n`, corrected to `no᷆k` on both sides of the fixture rather than treated as an `nj` case.
...
- `real_flex_export` is spec-derived from `phonetic2phonemic_public_test.csv`'s own etic and emic columns — 246 real word pairs, independently elicited, not the converter's own output. The fixture matches `Convert()`'s output exactly on all 246 rows. Three corrections were applied to the source data on the way in, rather than to the converter: the `ɡo᷅r**` ('payment') row's `**` (FLEx annotation noise on both columns, not phonemic content) stripped from both sides; `kɨ́ɾ wèɡbī` 'water yam' corrected to `kɨ́ɾ wə̀ɡbī` in the etic column, since it should have used `ɨ` rather than `e`, matching the corpus's own `wɨ̀ɡbī` → `wə̀ɡbī` 'dog' (the emic column, `kə́r wə̀ɡbī`, was already right); and `ɲo᷆k` → `no᷆k` 'to weave' on both sides, as described above.

  Six rows originally disagreed with `Convert()`'s output and were reviewed with the user (2026-09-01) rather than held out. Two are the `ɲ`/`nj` words above. `nzo᷇ɾ` 'chin' had an emic field (`nzo᷇r`) that predates the `nz → ndz` rule (assertion 3) being applied consistently; confirmed and now expected as `ndzo᷇r`. `kɨ́kjōɾākàp` 'river molluscs; shells' had an emic field (`kjukjōrɔ̄ kàp`) that read as a different transcription of the word rather than the regular rules applied to it; confirmed and now expected as the regular output, `kə́kjōrākàp`. The remaining two are the etic corrections above.
