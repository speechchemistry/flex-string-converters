...
**One more anomaly needs a different fix, because it doesn't fail the same way.** `ɡo᷅r**` 'payment' carries a trailing `**` on *both* the etic and emic columns — FLEx annotation noise, not phonemic content. Since etic and emic agree, it isn't one of the 5 mismatches above; but `**` is not in this converter's token lexicon either, so `convert()` raises `ValueError` on it, same as `phonemic2orthography.convert()` does. That matters mechanically: the CLI approval test in `zhire/tests/approval.py`-style feeds a whole fixture file to the converter's CLI as one `subprocess.run(..., check=True)` call, so one raising line kills that *entire file's* test with a `CalledProcessError` before any output is captured — hiding the diff for the other 245 words, not just skipping the bad one. That's a different failure mode from the 5 ordinary mismatches above, and defeats the point of reviewing a diff. **Recommendation: put this one row in its own tiny fixture file** (e.g. `real_flex_export_annotation_noise.txt`), so its crash stays isolated and the main file's mismatches stay visible. Also worth a line in `zhire/SPEC.md`, since it will surface again the first time this fixture is composed end-to-end with `phonemic2orthography.py`.
...
- `real_flex_export` — the real held-out net: 245 of the 246 rows of `phonetic2phonemic_public_test.csv`, columns 3–4 only. All 5 known mismatches (the 3 `ɲ` words plus the 2 data anomalies) are **left in this fixture on purpose**, per [5b](#open-questions) — they fail loudly with a normal received/approved diff for the user to review and fix in FLEx, rather than being held out or worked around with extra rules. **Specification-derived in the same sense as `phonology_sketch_examples`**: the emic column is independently elicited ground truth, not the converter's own output, so the promote loop is wrong for it too — write both sides directly from the CSV, never promote a received file into it. This is what caught the aspiration-deletion rule and the `ɲ`/`nj` split; see [Validation against real held-out data](#validation-against-real-held-out-data).
- `real_flex_export_annotation_noise` — the 246th row (`ɡo᷅r**` 'payment') on its own, for the mechanical reason in [5b](#open-questions): a raising row crashes its whole file's CLI approval test rather than producing a diffable mismatch, so it needs to be isolated to keep `real_flex_export`'s failures reviewable.

**An end-to-end composition test.** The most valuable check available, since the sketch supplies both ends: run each chart row's phonetic example through `phonetic2phonemic.convert()` then `phonemic2orthography.convert()` and compare against the sketch's orthography column. This is what produced the 44/47 above. Put it in `zhire/tests/`, and keep the 3 unresolved sketch rows (`nden`, `nggei`, `nzor`) out of it until the linguist has ruled on them, with a comment naming them rather than a silent omission — these are the sketch's own chart rows, a different set from the `real_flex_export` mismatches above, which stay in their fixture and are expected to fail.
...
- `zhire/tests/fixtures/phonetic2phonemic/{inputs,approved}/` — the fixtures above, including `real_flex_export.txt` and `real_flex_export_annotation_noise.txt`, both derived from `phonetic2phonemic_public_test.csv`.
...
5b. **Resolved 2026-08-27: keep known-anomalous rows in the fixture and let them fail**, rather than holding them out. User's instruction: "just let them fail, and I'll inspect the data and probably edit it. This is the same with other rare anomalies." Applies to the 2 genuine data anomalies (`kɨ́kjōɾākàp`, `kɨ́ɾ wèɡbī`) and, per decision 7 above, the 3 `ɲ` words too — all 5 stay in `real_flex_export` as ordinary mismatches with a reviewable diff. **One row is a partial exception on purely mechanical grounds, not a linguistic one**: `ɡo᷅r**` makes `convert()` raise rather than return a wrong value, and because the CLI approval test feeds a whole fixture file through one `subprocess.run(..., check=True)` call, that raise crashes the *entire file's* test and hides the diff for the other 245 words. Recommended default: put just this one row in its own tiny fixture file so it still fails loudly (satisfying "let it fail") without burying the other mismatches. Not a judgement call about the data — a consequence of how the CLI and approval harness are wired.
