...
### One source typo, corrected in the data rather than the converter

`[dzu᷈:ŋ]` 'permitted to go' writes the length mark as an ASCII colon `U+003A` instead of `U+02D0`. An earlier draft of this plan handled it with a `:` → `ː` rule in the converter. **That rule is gone.** It was the only entry in the whole mapping that existed to accommodate a mistake rather than to encode a real correspondence, and an audit of the rule set found nothing else in that category: every other rule implements a stated variant assertion, a correspondence the sketch's own charts demonstrate, or the removal of legitimate narrow-transcription detail.

An ASCII colon is not IPA, so `convert()` now raises on it, which is the correct answer for malformed input. The typo is fixed where it belongs — in the fixture, when it is built from the sketch — and reported upstream so the sketch itself gets corrected. Verified: the form appears exactly once in the sketch and nowhere in the real corpus, so this costs no coverage (still 183 of 183 forms once the fixture is corrected).
...
| 5 | Tone marks, `̃`, `ː`, space, and every phoneme letter kept as itself | FST identity arcs | Preserving a symbol is exactly what an identity arc does. |
| 6 | Reject anything unmapped, raising `ValueError` | **Falls out of the FST** | Composing an input containing an unmapped symbol yields an empty machine; `convert()` tests for that and raises. This is the *only* rejection mechanism — the modifier letters of [Modifier letters rejected](#modifier-letters-rejected-not-transliterated) are refused because the lexicon has no arc for them, not because a pre-pass screens for them. |
| 7 | Normalise to NFC | **Python post-pass** | Unicode hygiene on the way out, matching the sibling converter's contract. |

Note what is *not* in this table: there is no rule anywhere for the ASCII-colon typo in the sketch. Source-data mistakes are corrected in the data, not absorbed into the algorithm — see [One source typo](#one-source-typo-corrected-in-the-data-rather-than-the-converter) and [Corrections applied when building fixtures](#corrections-applied-when-building-fixtures).
...
Exactly two tokens are multi-character, `nz` and `ndʒ`, and each must beat its single-character parts — which maximal munch gives for free; the sibling converter's [correction 2](zhire-phonemic-to-orthography-fst.md#corrections-found-after-implementation) records where that behaviour actually comes from, and the same unweighted construction applies here. **Checked rather than assumed, per [AGENTS.md](../../AGENTS.md#external-specifications) on not claiming a row "falls out":** every other multi-letter sequence does fall out of the per-character arcs — `ndz` passes through unchanged, and `ɕw`, `ʑw`, `hw`, `ŋmɡb` are just their plain letters concatenated. Worth stating because `ndz` and `ndʒ` differ by one letter and only the second needs an entry: `ndz` must fall through untouched while `ndʒ` is rewritten.

### Verified, not designed on paper

The architecture above was built as a throwaway `pynini` lexicon and run against both corpora in both arrangements. Both reach the same totals — 240 of 246 real rows with **0 raises**, and 44 of 47 sketch chart rows — so the pre-pass change costs no coverage and is a pure correctness gain on the tie-bar cases, which neither corpus happens to contain. Also confirmed: rejection really does fall out of the empty-machine check with no pre-check present (`hʷók`, `ᵐbaŋ`, `tʲɛɾ`, `ᵑɡēj`, `q`, `ʈʂa` and the typo'd `dzu᷈:ŋ` all raise); the deletions work (`k͡pár` → `kpár`, `tsʼēn` → `tsēn`, `sīsʰi᷆p` → `sīsi᷆p`); and tone marks survive a `utf8`-token lexicon byte-identical.

**Not part of `convert()` at all.** Three transformations in this plan happen when a *fixture* is built, never at conversion time — the sketch's modifier letters, its ASCII-colon typo, and the `ɡo᷅r**` row's `**`. `convert()` raises on all three raw inputs, by design. They are collected in [Corrections applied when building fixtures](#corrections-applied-when-building-fixtures).
...
**Inline unit tests** (`zhire/tests/test_phonetic2phonemic.py`), one per general rule with the clearest single example, not duplicating what the corpus already covers: each of the five implemented variant rules (assertions 1–5, `ɲ` → `nj` included — see [Decisions](#decisions-resolved-2026-08-27)), `[ɨ]` → `/ə/`, release-mark deletion surviving a tie bar inside a multi-character token (the case [Why the release marks are stripped before the FST](#why-the-release-marks-are-stripped-before-the-fst) exists to cover), tone/nasalisation/length preservation, and the unmapped-input `ValueError` — covering a rejected modifier letter, a non-Zhire letter, and the sketch's ASCII colon, since all three are refused by the same mechanism.
...
### Corrections applied when building fixtures

Three transformations happen when a fixture is written and **never inside `convert()`**. Collected here because each one looks like it could have been a converter rule, and two of them were, in earlier drafts of this plan. `convert()` raises on all three raw inputs, which is the intended behaviour.

| Correction | Applies to | Why it isn't a converter rule |
| ---------- | ---------- | ----------------------------- |
| Modifier letters → plain letters (`ᵐb` → `mb`, `ᵑᵐɡb` → `ŋmɡb`, `tʲ` → `tj`, …) | 11 of 47 chart rows, 44 of 183 sketch forms | The notation asserts a syllable-structure interpretation that isn't settled; accepting it would commit the converter to an unfinished analysis. [Modifier letters rejected](#modifier-letters-rejected-not-transliterated) |
| ASCII colon `:` → `ː` | 1 sketch form, `[dzu᷈:ŋ]` | A typo, not a correspondence. The only accommodation-for-bad-data rule the set ever had, now removed. [One source typo](#one-source-typo-corrected-in-the-data-rather-than-the-converter) |
| Strip `**` | 1 real-corpus row, `ɡo᷅r**` | FLEx annotation noise on both columns, not phonemic content. |

All three should also be reported upstream so the sources themselves get fixed; a fixture correction is a local workaround, and `zhire/SPEC.md` should record each one so the fixtures can be re-derived when the sources change.
